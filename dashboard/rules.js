...
function initConditions() {
  ...
  //   if (valueOptions[conditionType]?.[fieldValue]) {
  //     valueOptions[conditionType][fieldValue].forEach(val => {
...
function resetForm() {
  ...
  const conditionList = document.querySelector(".condition-list");
  conditionList.innerHTML = '';  //remove all existing rows
}
...
function getRulesFromStorage() { //read data
...
function saveRule(rule) {  //get current rules from storage
  const rules = getRulesFromStorage();
...
function loadRules() { //load and display rules
  const rules = getRulesFromStorage();
  displayRules(rules);
}
...
deleteIcon.src = "../asset/delete.png"; // <-- your delete icon path
...
editIcon.src = "../asset/edit.png"; // <-- your edit icon path
editIcon.alt = "Edit";
